...
//   ✅ App Registration (ClientSecretCredential) → bearer token for ARM / Data API
...
using Komatsu.ApimMarketplace.Bff.Authorization;
using Komatsu.ApimMarketplace.Bff.Endpoints;
using Komatsu.ApimMarketplace.Bff.Middleware;
...
builder.Services.Configure<ServicePrincipalSettings>(builder.Configuration.GetSection($"{ApimSettings.SectionName}:ServicePrincipal"));
builder.Services.Configure<GlobalAdminSettings>(builder.Configuration.GetSection(GlobalAdminSettings.SectionName));

// RBAC policies — hot-re loadable from rbac-policies.json
...
// Data API HttpClient — used when UseDataApi=true (runtime mode)
...
// Global Admin API HttpClient — fetches user business roles (Distributor/Vendor/Customer)
...
// ─── IHttpContextAccessor (needed by DataApiService for user-scoped requests) ─
builder.Services.AddHttpContextAccessor();

// ─── Role Provider: Global Admin API (fetches Distributor/Vendor/Customer) ──────
// In mock mode, every user gets Distributor role with all permissions.
// In production, roles are fetched from Global Admin API and cached 30 min.
if (useMockMode)
{
...
// ─── Token Provider: App Registration (client credentials flow) ──────────────
// All outbound ARM / Data API calls use a single service-principal identity.
// The BFF has full access to all APIM APIs; RBAC filtering happens in the
// authorization handler based on Global Admin roles.
// In mock mode, the token provider is not registered (no live API calls).
if (!useMockMode)
{
    builder.Services.AddSingleton<ITokenProvider, AppRegistrationTokenProvider>();
}

// ─── Services: mock vs. Data API vs. ARM ─────────────────────────────────────

var apimSettings = builder.Configuration.GetSection(ApimSettings.SectionName).Get<ApimSettings>();
...
{
    // Runtime mode: use APIM Data API (flat responses, user-scoped prefixing)
    builder.Services.AddScoped<IArmApiService, DataApiService>();
}
else
{
    // Design-time mode: use ARM Management API (properties envelope unwrapping)
    builder.Services.AddScoped<IArmApiService, ArmApiService>();
}

// ─── Legacy API Support ──────────────────────────────────────────────────────
// Enables integration with legacy SOAP, binary, and custom protocol APIs
// Provides unified catalog with cloud APIM APIs

builder.Services.Configure<SoapLegacySettings>(
    builder.Configuration.GetSection("LegacyApis:Soap"));
...
// HTTP client for legacy API calls (SOAP endpoint, auth endpoint, etc.)
...
app.Lifetime.ApplicationStarted.Register(() =>
{
    ...
    app.Logger.LogInformation("API Mode:      {Mode}", useMockMode ? "Mock" : useDataApi ? "Data API (runtime)" : "ARM Management API");
    app.Logger.LogInformation("Auth:          {Mode}", useMockMode ? "Mock Mode (Development)" : "JWT → Global Admin roles → RBAC");
    if (!useMockMode)
    {
        var gaSettings = builder.Configuration.GetSection(GlobalAdminSettings.SectionName).Get<GlobalAdminSettings>();
        app.Logger.LogInformation("Global Admin:  {Url} (cache {Min} min)", gaSettings?.BaseUrl ?? "(not configured)", gaSettings?.RoleCacheMinutes ?? 30);
    }
    if (useDataApi && !string.IsNullOrWhiteSpace(settings.DataApiUrl))
        app.Logger.LogInformation("Data API:      {Url}", settings.DataApiUrl);
...
namespace Komatsu.ApimMarketplace.Bff { public partial class Program; }
