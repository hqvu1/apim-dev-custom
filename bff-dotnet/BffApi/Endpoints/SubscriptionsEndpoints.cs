...
public static class SubscriptionsEndpoints
{
    public static RouteGroupBuilder MapSubscriptionsEndpoints(this WebApplication app)
    {
        ...
        // POST /subscriptions/{subId}/regeneratePrimaryKey — regenerate primary key
        group.MapPost("/{subId}/regeneratePrimaryKey", async (string subId, IArmApiService svc, CancellationToken ct) =>
        {
            var sub = await svc.RegeneratePrimaryKeyAsync(subId, ct);
            return sub is null ? Results.NotFound() : Results.Ok(sub);
        })
        .WithName("RegeneratePrimaryKey")
        .WithSummary("Regenerate subscription primary key")
        .RequireAuthorization("ApiSubscribe")
        .Produces<SubscriptionContract>()
        .Produces(StatusCodes.Status404NotFound);

        // POST /subscriptions/{subId}/regenerateSecondaryKey — regenerate secondary key
        group.MapPost("/{subId}/regenerateSecondaryKey", async (string subId, IArmApiService svc, CancellationToken ct) =>
        {
            var sub = await svc.RegenerateSecondaryKeyAsync(subId, ct);
            return sub is null ? Results.NotFound() : Results.Ok(sub);
        })
        .WithName("RegenerateSecondaryKey")
        .WithSummary("Regenerate subscription secondary key")
        .RequireAuthorization("ApiSubscribe")
        .Produces<SubscriptionContract>()
        .Produces(StatusCodes.Status404NotFound);
